        ...
        if initializedSounds
            ...
            if doneTraining
                ...
                if (TapReceived && AcceptTaps) || (ongoingExperiment && currentExpStep > 6)
                    ...
                    %curr
                    expectedNumber = currentExpStep - 6;
...
function Key_Down(~, event)
    ...
    if isempty(event.Modifier) && strcmp(event.Key, 'space')
        ...
        if ongoingExperiment
            if isempty(ReplayMode)
                set(handles.debugMessage, 'string', sprintf('Stopping experiment %d', currentExpStep));
                drawnow;
            end
            ...
            if isempty(ReplayMode) && currentExpStep <= 5
                onDeck = expSteps(currentExpStep+1);
                if strcmp(get(onDeck, 'type'), 'line')
                    set(onDeck, 'color', PREP_COLOR);
                else
                    set(onDeck, 'MarkerFaceColor', PREP_COLOR);
                    set(onDeck, 'MarkerEdgeColor', PREP_COLOR);
                end
            end
            ...
            if isempty(ReplayMode)
                set(handles.debugMessage, 'string', sprintf('Starting experiment %d', currentExpStep));
                drawnow;
            end
            
            
            if isempty(ReplayMode) && currentExpStep <= 6
