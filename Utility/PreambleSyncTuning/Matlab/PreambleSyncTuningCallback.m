function [] = AppPreambleSyncTuningCallback( obj, type, data )
%SERVERDEVCALLBACK Summary of this function goes here
    ...
    if type == obj.CALLBACK_TYPE_DATA,
        ...
        if obj.userfig == -1, % need to create a new UI window
            ...
            [maxValues, maxIdxs] = max(abs(cons(:, :, detectChIdx)), [],1);
            detectResultNow = squeeze(maxIdxs);
            ...
            if check3.Value == 1,
                ...
                detectResultsEnd = detectResultsEnd+nowSize;
                set(line, 'yData', detectResults - 600); % only show the 1st ch
